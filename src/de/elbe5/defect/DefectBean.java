...
public class DefectBean extends ContentBean {
    ...
    private static final String GET_CONTENT_EXTRAS_SQL = "SELECT position_comment, location, remaining_work, assigned_id, project_phase, notified, " +
            "position_x, position_y, due_date1, due_date2, close_date  " +
        ...
        try {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    data.setLocation(rs.getString(i++));
                    data.setRemainingWork(rs.getBoolean(i++));
    ...
    private static final String INSERT_CONTENT_EXTRAS_SQL = "insert into t_defect (" +
            "position_comment, location, remaining_work, assigned_id, project_phase, notified, " +
            "due_date1, " +
    ...
    public void createContentExtras(Connection con, ContentData contentData) throws SQLException {
        ...
        try {
            ...
            pst.setString(i++, data.getLocation());
            pst.setBoolean(i++, data.isRemainingWork());
    ...
    private static final String UPDATE_CONTENT_EXTRAS_SQL = "update t_defect " +
            "set position_comment=?, location=?, remaining_work=?, assigned_id=?, project_phase=?, notified=?, due_date1=?, due_date2=?, position_x=?, position_y=? where id=? ";
