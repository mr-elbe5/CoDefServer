...
public class UnitCsvCreator extends CsvCreator {
    ...
    static String[] unitFields = {"_project", "_unit"};
    ...
    private void writeHeader(CSVWriter writer){
        ...
        list.add(csv(unit.getProject().getDisplayName()));
    ...
    private void writeDefectStatus(CSVWriter writer, DefectStatusData status){
        List<String> list = new ArrayList<>();
        for (int i = 0; i < unitFields.length + defectFields.length; i++)
            list.add("");
